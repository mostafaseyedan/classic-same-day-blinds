...
export default function CartRecommendations({ cart, language, onAddToCart }: CartRecommendationsProps) {
  ...
  const otherProducts = productCatalog.filter(
    (p) => !cartIds.has(p.id) && !cartCategories.includes(p.category)
  );
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">
            {language === 'es'
              ? 'Productos seleccionados basados en tu carrito'
              : "Handpicked based on what's in your cart"}
          </p>
        </div>
        <Link
          to="/products"
