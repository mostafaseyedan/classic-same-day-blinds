...
export default function CompanyDetailPage() {
  ...
  const handleEdit = (updated: any) => {
    const companies = loadCompanies();
    const updatedList = companies.map((c: any) => String(c.id) === String(id) ? { ...c, ...updated } : c);
    localStorage.setItem('admin_companies', JSON.stringify(updatedList));
    setCompany((prev: any) => ({ ...prev, ...updated }));
  };

  const handleDelete = () => {
    const companies = loadCompanies();
    const updated = companies.filter((c: any) => String(c.id) !== String(id));
    ...
    return (
      <div className="flex flex-col items-center justify-center h-screen bg-slate-50 gap-4">
        <div className="w-16 h-16 bg-slate-100 rounded-2xl flex items-center justify-center">
          <i className="ri-building-4-line text-slate-400 text-3xl"></i>
        </div>
        <div className="text-center">
          <p className="text-lg font-bold text-slate-900">Company Not Found</p>
          <p className="text-sm text-slate-500 mt-1">Company #{id} does not exist.</p>
        </div>
        <button
          onClick={() => navigate('/admin/companies')}
          className="flex items-center gap-2 px-5 py-2.5 bg-slate-900 text-white text-sm font-bold rounded-xl cursor-pointer"
        >
          <i className="ri-arrow-left-line"></i> Back to Companies
        </button>
      </div>
    );
  }
