...
const OUTCOMES = [
  { id: 'spoke', label: 'Spoke with customer', icon: 'ri-phone-line', color: 'bg-emerald-100 text-emerald-700 border-emerald-300' },
  ...
  { id: 'callback', label: 'Customer to call back', icon: 'ri-phone-forward-line', color: 'bg-sky-100 text-sky-700 border-sky-300' },
];
...
export default function LogCallModal({ customer, onClose, onLogged }: Props) {
  ...
  const handleLog = () => {
    ...
    setTimeout(() => {
      ...
      let desc = `${direction === 'outbound' ? 'Outbound' : 'Inbound'} call — ${outcomeLabel}`;
      if (notes.trim()) desc += `. ${notes.trim()}`;
      if (followUp.trim()) desc += ` | Follow-up: ${followUp.trim()}`;
      ...
      saveActivity(customer.id, {
        ...
        color: outcome === 'spoke' ? 'bg-emerald-600' : outcome === 'voicemail' ? 'bg-amber-500' : 'bg-slate-500',
        meta: durationStr ?? outcomeLabel,
      });
      setSaving(false);
      setSaved(true);
      setTimeout(() => {
    ...
    <div className="fixed inset-0 bg-black/50 z-[60] flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-slate-100">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">{customer.firstName} {customer.lastName} &bull; {customer.phone || customer.email}</p>
            </div>
          </div>
        ...
        <div className="p-6 space-y-5">
          ...
          <div>
            ...
            <div className="flex items-center gap-2">
              <input
                ...
                placeholder="0"
                className="w-24 border border-slate-200 focus:border-slate-400 rounded-lg px-3 py-2.5 text-sm outline-none text-center text-slate-700"
              />
          ...
          <div>
            ...
            <input
              ...
              placeholder="e.g. Send updated quote by Friday, Schedule demo..."
        ...
        <div className="px-6 py-4 border-t border-slate-100 flex gap-3 justify-end">
          ...
          >
            {saved ? (
              <><i className="ri-check-line text-base"></i> Logged!</>
            ) : saving ? (
              <><i className="ri-loader-4-line text-base animate-spin"></i> Saving…</>
            ) : (
